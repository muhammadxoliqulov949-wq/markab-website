...
/**
 * Trust layer — a compact credibility strip, not a feature grid.
 *
 * Verified information only: no invented certifications, no invented customer
 * counts, no invented statistics. Where a claim exists publicly but has no
 * published evidence (AAOIFI), the gap is stated on the item itself instead of
...
const facts = [
  {
    ...
    icon: (
      <path d="M12 3l7 3v5.5c0 4.3-2.9 7.6-7 9.5-4.1-1.9-7-5.2-7-9.5V6l7-3Zm-1 11.2 4.5-4.5-1.4-1.4L11 11.4l-1.1-1.1-1.4 1.4L11 14.2Z" />
    ),
  },
  {
...
export function TrustLayer() {
  return (
    <section aria-labelledby="trust-heading" className="border-b border-line bg-surface-muted">
      <Container className="py-8 sm:py-10">
        <h2 id="trust-heading" className="sr-only">
          Nega Markabga ishonish mumkin
        </h2>

        <ul className="grid gap-0 divide-y divide-line sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-4">
          {facts.map((fact, index) => (
            <li
              key={fact.title}
              className={[
                'flex gap-3 py-4 sm:px-5 sm:py-0',
                index === 0 ? 'sm:pl-0' : '',
                'sm:border-l sm:border-line first:sm:border-l-0',
              ].join(' ')}
            >
              <svg
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-sm leading-relaxed text-ink-600">{fact.detail}</p>
                {fact.note ? (
