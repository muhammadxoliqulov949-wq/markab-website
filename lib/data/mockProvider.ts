...
export const mockProvider: DataAdapter = {
  ...
  async searchCatalogue(
    ...
    const index = buildSearchHits();

    const rank = <T extends SearchHit>(
      items: { hit: T; haystack: string[] }[],
    ): T[] =>
      items
        ...
        .sort(
          (a, b) => b.score - a.score || a.hit.title.localeCompare(b.hit.title) || a.hit.id.localeCompare(b.hit.id),
        )
        .map((entry) => entry.hit);
    ...
    const productMatches = rank(
      index.products.map((hit, i) => ({
        hit,
        haystack: [hit.title, hit.subtitle, products[i].rawTitle, products[i].category],
      })),
    );
    ...
    return success<CatalogueSearchResults>({
      query: trimmed,
...
/**
 * Deterministic match quality for search.
 *
 * This is NOT relevance ranking and must never be described as one: it is a
 * fixed three-step ladder (whole-field prefix → word prefix → substring) with
 * alphabetical and id tie-breaks, so the same query always returns the same
...
function buildSearchHits(): { vehicles: SearchHit[]; products: SearchHit[] } {
  return {
