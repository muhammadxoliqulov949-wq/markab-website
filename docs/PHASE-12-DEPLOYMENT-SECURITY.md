...
| A5 | Confirm `Cross-Origin-Resource-Policy: same-origin` does not break legitimate embeds | **S** | **Checked:** the only embed anywhere is the contact page's map iframe (Google Maps embed, `components/contact/OfficeMap.tsx`) — a third-party *response* our visitors load inside an iframe we control, not an embedding of *our* responses. Every other external destination is an `<a>` with `rel="noopener noreferrer"` (app stores, Google Maps deep link, markab.uz/privacy). CORP constrains who may embed our responses, not what we load, so the map embed and catalogue photography are unaffected. Re-check if another embed or a marketing tag is ever added. |
